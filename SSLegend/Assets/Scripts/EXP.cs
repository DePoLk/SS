...
public class EXP : MonoBehaviour {
    ...
    {         
            this.GetComponent<SphereCollider>().isTrigger = true;
            radian += perRadian;
            float dy = Mathf.Cos(radian) * radius; // 修改dy改變浮動率
            transform.position = OriPos + new Vector3(0, dy * 0.5f, 0);
            IsFloating = true;
    }
    ...
    void GetExp()
    {
        if (WaitTime <= 0)
        {
            ...
            WaitTime = 0;
                ...
                //this.GetComponent<Rigidbody>().useGravity = false;
                //this.GetComponent<SphereCollider>().isTrigger = true;
                PlayerFixPos = player.transform.position + new Vector3(0f, 1f, 0f);
                //CheckPlayerPos();
                //Vector3 move = Vector3.Slerp(this.transform.position,PlayerFixPos, smoothSpeed);
                //this.transform.position = move;
                this.transform.DOLocalMove(PlayerFixPos,0.5f);
